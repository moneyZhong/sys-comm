...
public class ExcelUtil {
     ...
     * @return
     */
    public static <T extends BaseRowModel> List<T> readExcel(InputStream inputStream,Class<T>  rowModel) {
        ExcelListener excelListener = new ExcelListener();
    ...
    /**
     * 读取输入流，表头rowNum,解析单sheet页excel，返回指定class List
     * @param inputStream
     * @param rowModel
     * @param headLineNum
     * @param <T>
     * @return
     */
    public static <T extends BaseRowModel> List<T> readExcel(InputStream inputStream, Class<T>  rowModel, int headLineNum) {
        ExcelListener excelListener = new ExcelListener();
        ExcelReader reader = getReader(inputStream, excelListener);
        if (reader == null) {
            return new ArrayList<>();
        }
        reader.read(new Sheet(1, headLineNum, rowModel));
        return getExtendsBeanList(excelListener.getDatas(),rowModel);
    }
    /**
     * 利用BeanCopy转换list
